...
This version:
    - Does NOT require validate_real_landmarks.py
    - Builds the MediaPipe HandLandmarker directly
    - Extracts 126-D landmarks directly
    - Matches the current GestureClassifier in train_classifier.py
...
def send_to_firestore(sentence):
    """
    Send the completed sentence to Firestore.
    """
...
def build_hand_landmarker(
    ...
    the detector should not disappear too easily during
    fast/sudden gestures.
    """
...
def landmarks_to_vector(result):
    ...
    Handedness is used to place each hand into a
    deterministic Left/Right slot.
    """
    ...
    if (
        result is None
        or not hasattr(result, "hand_landmarks")
    ...
    for hand_idx, hand_landmarks in enumerate(
        ...
        for landmark_idx, lm in enumerate(
            hand_landmarks
        ):
    ...
    # Overall confidence
    #
    # MAX is used because one clearly visible hand should
    # remain usable even if the second hand is poorly detected.
    # --------------------------------------------------------
...
def run(
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            # ------------------------------------------------
            # EXTRACT 126-D LANDMARK FEATURES
            # ------------------------------------------------
            ...
            if nu_t < neutral_threshold:
                ...
                # ---------------------------------------------
                # GestureClassifier architecture
                #
                # h_t
                #   -> fc1
                #   -> ReLU
