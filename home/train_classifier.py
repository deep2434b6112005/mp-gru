...
Changes from the previous version (aimed at lifting val_acc):
  ...
  2. Added an auxiliary standard cross-entropy classifier head trained
     ...
     convergence a lot.
  3. Added optional light on-the-fly augmentation for landmark sequences
     (per-sample rotation/scale jitter + additive noise + random
     temporal masking), applied only when --augment is on and only
     during training. It is deliberately off for the initial clean
     baseline, then can be evaluated as a separate experiment.
...
def main():
    ...
    parser.add_argument("--class-weights", action="store_true",
                         help="Enable inverse-frequency class weighting (off for the clean baseline)")
    parser.add_argument("--no-class-weights", action="store_true",
                         help="Deprecated compatibility flag; class weighting is off by default")
    ...
    parser.add_argument("--prototype-ema", action="store_true",
                         help="Enable EMA updates for the prototype table (off for the clean baseline)")
    parser.add_argument("--augment", action="store_true", default=False,
                         help="Apply landmark augmentation during training (off for the clean baseline)")
    parser.add_argument("--no-augment", dest="augment", action="store_false",
                         help="Disable landmark augmentation (the default)")
    parser.add_argument("--temporal-warp", action="store_true", default=False,
                         help="Apply random speed-warping augmentation (off for the clean baseline)")
    parser.add_argument("--no-temporal-warp", dest="temporal_warp", action="store_false",
                         help="Disable temporal warping (the default)")
    ...
    # Kept so older launch commands remain valid now that the clean baseline
    # defaults to no class weighting.
...
if __name__ == "__main__":
    main()
""""python train_classifier.py --cache cache --epochs 60 --batch-size 8 --augment --temporal-warp --class-weights --prototype-ema
run this command"""